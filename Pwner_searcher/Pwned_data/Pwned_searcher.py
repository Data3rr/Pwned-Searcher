...
def main():
    ...
    #retieve json params
    with open('app/pwned_config.json', 'r', encoding='utf-8') as config:
...
╚═╝      ╚══╝╚══╝ ╚═╝  ╚═══╝╚══════╝╚═════╝""" + y + """ by Adapters""" + w).replace('█', w + '█' + y))
    print(f"""\n{y}[{w}+{y}]{w} Menu:              |    {y}[{w}INFO{y}]{w} Current Settings:            
                       |
{y}[{w}1{y}]{w} Start Pwned        |    {y}[{w}>{y}]{w} Allocated ram: {params['allowed-ram'] + 'go' if params['allowed-ram']  != '' else 'Undefined'}     
{y}[{w}2{y}]{w} Change setup       |    {y}[{w}>{y}]{w} Undef: .""")      
    ...
    if general_choice == '1':
        ...
        with open('app/pwned_config.json', 'w', encoding='utf-8') as config:
